...
def memepie():
    if 'meme' in request.values:
        ...
        if meme.is_valid():
            twitter = Twitter()
            parser = Parser()
            sources = twitter.get_sources(meme, 20)
            #from source import Source
            #sources = [Source("", "x is my middle name",""), Source("", "y is my middle name", ""), Source("", "x is my middle name","")]
            result = parser.collate_words(meme, sources)
